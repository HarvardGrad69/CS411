...
if (mysqli_connect_errno()) {
  ...
  $qry =  mysqli_query($connect, "CREATE PROCEDURE dit(@keyword INT) AS BEGIN DECLARE @keyword AS INT SELECT crime.ID, Arrest, crime.Description, Datetime, Neighbourhood FROM crime, location, date WHERE crime.LocationID = location.ID and crime.DateID = date.ID and location.neighbourhood=@keyword END");
