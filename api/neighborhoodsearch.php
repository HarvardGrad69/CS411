...
if (mysqli_connect_errno()) {
  ...
  $rawr = mysqli_query($connect, "CREATE PROCEDURE `gadf`($keyword) SELECT crime.ID, Arrest, crime.Description, Datetime, Neighbourhood FROM crime, location, date WHERE crime.LocationID = location.ID and crime.DateID = date.ID and location.neighbourhood=$keyword");
  if (!$rawr) {
